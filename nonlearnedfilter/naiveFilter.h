...
class NaiveFilter{
    public:
        NaiveFilter(double bitsPerItem, size_t pos_num, double usedNegRatio, std::vector<Slice *> &neg_keys_);
        ~NaiveFilter();

        void Add(Slice & key);
        bool Query(Slice & key);
        
        void PrintHaveInsertNum()
        {
            std::cout<<"HaveInsertPosNum: "<<haveInsertNum<<endl;
        }

    private:
        CountingBloom * neg_filter_; //used to store negative items
        CountingBloom * pos_filter_; //used to store positive items which do not collide with those negative items; 
        CountingBloom * coll_filter_; //used to store positive items which collide with those negative items; 
        double sumSpace_; 
        double sizeRatio_filter3_;  //The ratio of space on the pos_filter_small;
        size_t bitPerCounter_;
        double usedNegRatio_;
        size_t haveInsertNum;
};

NaiveFilter::NaiveFilter(double bitsPerItem, size_t pos_num, double usedNegRatio, std::vector<Slice *> &neg_keys_)
{   
    sumSpace_ = bitsPerItem * pos_num;
    sizeRatio_filter3_ = 0.1;
    bitPerCounter_ = 4;
    usedNegRatio_ = usedNegRatio;
    double space_filter3 = sumSpace_ * sizeRatio_filter3_;
    double space_filter1 = (sumSpace_ - space_filter3) / 2.0;
    haveInsertNum = 0;
    size_t numCounter_negFilter =  space_filter1 / bitPerCounter_;
    size_t numCounter_CollFilter =  space_filter3 / bitPerCounter_;

    size_t nHashFunc_posFilter = (double)space_filter1 / bitPerCounter_ / pos_num * log(2);
    std::cout<<"The number of hashFun(posFilter): "<<nHashFunc_posFilter<<endl;
    size_t nHashFunc_CollFilter = 3;

    neg_filter_ = new CountingBloom( numCounter_negFilter, bitPerCounter_, nHashFunc_posFilter);
    pos_filter_ = new CountingBloom( numCounter_negFilter, bitPerCounter_, nHashFunc_posFilter);
    coll_filter_ = new CountingBloom( numCounter_CollFilter, bitPerCounter_, nHashFunc_CollFilter);

    size_t saveNegNum = usedNegRatio_ * neg_keys_.size();
    for (size_t i = 0; i < saveNegNum; i++)
    {
        neg_filter_->counting_bloom_add(*neg_keys_[i]);
    }
...
bool NaiveFilter::Query(Slice & key)
{
    if(neg_filter_->counting_bloom_check(key)){
        return coll_filter_->counting_bloom_check(key);
    }else{
        return pos_filter_->counting_bloom_check(key);
    }
}

void NaiveFilter::Add(Slice & key)
{
    if (neg_filter_->counting_bloom_check(key))
    {
        coll_filter_->counting_bloom_add(key);
    }else{
        pos_filter_->counting_bloom_add(key);
    }
    haveInsertNum++;
}
